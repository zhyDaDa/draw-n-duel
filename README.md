...
| 流派名称   | 核心思路 (Playstyle)                                                                                       | 关键词 (Keywords)          | 优势                                 | 劣势                                               | 核心卡牌设计方向                                                                                                                                                                                                                                                                                                                                       | 适配的大奖                             |
| ---------- | ---------------------------------------------------------------------------------------------------------- | -------------------------- | ------------------------------------ | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| 基础运算流 | 通过直接、稳定的数学运算（加减乘除、平方、开方）来积累高分，以绝对的分数优势取胜。                         | 基础, 稳定                 | 稳定，下限高，简单直接。             | 容易被干扰，缺乏特殊胜利手段。                     | - 自身运算: 分数+N, 分数-N, 分数xN, 分数÷N, 分数平方, 分数开方。<br>- 高额加分: 提供比 连锁加法 更高的基础分值。rare卡可以提供加分的增益, 以及防止分数变小的盾牌buff                                                                                                                                                                                   | 稳定增益: 每层初始分数+8               |
| 胜利碎片流 | 放弃部分分数博弈，专注于收集特定种类的“胜利碎片”，集齐一套即可直接获胜。是一种高风险、高回报的“赌狗”玩法。 | 碎片, 收藏, 风险, 特殊胜利 | 无视分数，一旦成型直接胜利。         | 极不稳定，容易“卡手”，收集过程中自身分数可能很低。 | - 碎片种类: 设计多种碎片，如乱字碎片(形如"利片胜碎")或东/南/西/北风碎片，需集齐同系列。<br>- 获取卡: 放弃N次抽卡，获得1枚指定碎片、当分数低于10时，获得1枚随机碎片、弃掉所有滞留卡，获得1枚碎片。<br>- 对于乱字碎片, 可以有legendary卡"修正器"(可以放在商人大奖里), 使得每次的乱序自动变成正常顺序, 加速收集, (注意, 收集三个一样的乱序也是可以获胜的) | 修正器: 乱字碎片自动变正常顺序         |
| 干扰破坏流 | 核心不是为自己加分，而是不择手段地削弱、限制甚至摧毁对手的分数。让对手无法正常游戏。                       | 干扰, 破坏, 削弱, 恶作剧   | 极大限制对手发育，创造翻盘机会。     | 自身得分能力弱，若对手有防御手段则效果大减。       | - 分数破坏: 对手分数归零、对手分数x(-1)（若规则允许负分）、偷取对手N%的分数。<br>- 规则限制debuff: 对手下一回合无法使用滞留卡、随机丢弃对手一张滞留卡。                                                                                                                                                                                                | 毒药: 弃牌会使得其余玩家-2分           |
| 滞留连击流 | 利用滞留位（Hold Slots）作为“手牌”，通过卡牌之间的配合打出强大的 Combo 效果，一回合内爆发极高分数。        | 滞留, 连击, 组合技         | 爆发力强，一回合可逆转局势。         | 启动慢，需要凑齐关键组件，滞留位被破坏则体系崩溃。 | - 滞留增益: 滞留位每有一张牌，本卡效果+N。<br>- 连击触发: 若本回合已使用过滞留卡，则...。<br>- 效果复制: 复制并结算最顶部的滞留卡效果（已有的滞留增幅器）。                                                                                                                                                                                            | 连击大师: 滞留与释放的操作次数翻倍累积 |
| 无限抽卡流 | 通过大量抽牌，将抽卡本身作为一种资源进行利用，形成强大的卡牌组合。                                         | 抽牌, 循环, 资源           | 抽到组件直接无敌, 浪费卡组来干扰对手 | 卡牌本身没有分数，需要有效转化资源为分数。         | - 额外抽牌: 获得N次额外抽卡机会（已有的归零试胆等）。<br>- 抽卡盛宴: 额外增加3次抽卡, 但后面三次抽卡的数值(若有)全部归零<br>- 奢华: 抽取一张卡牌, 若是普通卡扣2分并丢弃, 重复这一效果直到抽到rare及以上<br> -资源汇集: 放弃剩余的所有抽卡次数(n), 获取 n^2的分数                                                                                       | 抽卡之王: 每层初始分数置为-1,抽卡+3    |
