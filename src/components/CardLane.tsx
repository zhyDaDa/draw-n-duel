...
export type CardLaneAnimationType = 'draw' | 'stash' | 'discard' | 'play' | 'release'
...
const renderCard = (
  ...
  return (
    <Tooltip title={renderTooltip(card)} placement="top" overlayClassName="tooltip-light">
...
const CardLane: React.FC<CardLaneProps> = ({
  ...
  const slotCards = useMemo(() => (
    Array.from({ length: maxHoldSlots }).map((_, index) => holdSlots[index] ?? null)
  ...
  const holdClasses = useMemo(() => (
    slotCards.map((card, index) => {
      ...
      if (animationEvent.type === 'stash' && animationEvent.card.instanceId === card.instanceId && index === 0) {
  ...
  return (
    <section className="card-lane" aria-label="卡牌分区">
      ...
      <div className={`card-slot-group ${stackShiftKey ? 'card-slot-group--animated' : ''}`}>
        {slotCards.map((card, index) => (
          <div key={card ? card.instanceId : `slot-${index}`} className="card-slot card-slot--hold">
            {card ? (
              renderCard(card, { extraClass: holdClasses[index] })
            ) : (
              <div className="card-slot__placeholder card-slot__placeholder--hold">滞留位空</div>
            )}
